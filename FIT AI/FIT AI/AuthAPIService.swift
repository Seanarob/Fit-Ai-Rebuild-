...
struct AuthAPIService {
    ...
    func register(email: String, password: String) async throws -> AuthRegisterResponse {
        ...
        do {
            ...
            // Fallback: try to extract userId from response
            if let jsonObject = try? JSONSerialization.jsonObject(with: data, options: []),
               let payload = jsonObject as? [String: Any] {
                if let userId = payload["user_id"] as? String ?? payload["userId"] as? String {
